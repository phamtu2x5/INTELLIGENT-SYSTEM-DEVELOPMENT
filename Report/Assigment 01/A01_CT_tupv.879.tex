...
\begin{tcolorbox}[colback=blue!3!white, colframe=blue!75!black, title=\textbf{Kho lưu trữ Mã nguồn \& Jupyter Notebook Thực nghiệm (GitHub Repository)}]
\small
\begin{itemize}[leftmargin=1.5em, itemsep=3pt]
    ...
    \item \textbf{Mã nguồn Notebook Hệ thống Chẩn đoán Tiểu đường (Week 1):} \href{https://github.com/phamtu2x5/INTELLIGENT-SYSTEM-DEVELOPMENT/blob/main/src/Week%201/notebooks/01_diabetes_analysis.ipynb}{\texttt{01\_diabetes\_analysis.ipynb}}
    \item \textbf{Mã nguồn Notebook Hệ thống Định giá Bất động sản (Week 2):} \href{https://github.com/phamtu2x5/INTELLIGENT-SYSTEM-DEVELOPMENT/blob/main/src/Week%202/house_price_prediction/notebooks/01_house_price_analysis.ipynb}{\texttt{01\_house\_price\_analysis.ipynb}}
\end{itemize}
\end{tcolorbox}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item \textbf{Mã nguồn Notebook Huấn luyện \& Thực nghiệm (Week 1):} \href{https://github.com/phamtu2x5/INTELLIGENT-SYSTEM-DEVELOPMENT/blob/main/src/Week%201/notebooks/01_diabetes_analysis.ipynb}{\texttt{01\_diabetes\_analysis.ipynb}}
    \item \textbf{Nguồn gốc Dữ liệu trên Kaggle:} \href{https://www.kaggle.com/datasets/uciml/pima-indians-diabetes-database}{\texttt{Kaggle -- Pima Indians Diabetes Database}}
    \item \textbf{Kho lưu trữ UCI Machine Learning Repository:} \href{https://archive.ics.uci.edu/dataset/34/diabetes}{\texttt{UCI ML Repository -- Diabetes Dataset}}
\end{itemize}
...
\toprule
\textbf{Cấu hình XGBoost} & \textbf{Train Acc (\%)} & \textbf{Test Acc (\%)} & \textbf{Precision (\%)} & \textbf{Recall (\%)} & \textbf{F1-Score} & \textbf{ROC-AUC} \\
\midrule
...
\textbf{2. Thiết kế Không gian Biểu diễn Đối chứng:}
...
\textbf{3. Kết quả Đối chứng Không gian Biểu diễn:}
...
\begin{tabular}{lccc}
\toprule
\textbf{Mô hình Học máy} & \textbf{Thang đo Thô ($X_{\text{unscaled}}$)} & \textbf{Chuẩn hóa ($X_{\text{scaled}}$)} & \textbf{Kỹ nghệ ($X_{\text{engineered}}$)} \\
\midrule
\textbf{K-Nearest Neighbors (KNN)} & $67.53\%$ ($0.7120$) & $70.78\%$ ($0.7850$) & \textbf{71.43\% (0.7980)} \\
\textbf{Support Vector Machine (SVM)} & $64.94\%$ ($0.5820$) & $73.38\%$ ($0.7910$) & \textbf{74.03\% (0.7976)} \\
\textbf{Logistic Regression} & $70.13\%$ ($0.8090$) & $70.78\%$ ($0.8130$) & \textbf{70.78\% (0.8130)} \\
\textbf{Random Forest Classifier} & $71.43\%$ ($0.8190$) & $71.43\%$ ($0.8190$) & \textbf{71.43\% (0.8230)} \\
\textbf{XGBoost (Tuned)} & $74.68\%$ ($0.8150$) & $74.68\%$ ($0.8150$) & \textbf{75.32\% (0.8289)} \\
\bottomrule
\end{tabular}
...
\textbf{4. Nhận xét \& Ý nghĩa Khoa học:}
\begin{itemize}[leftmargin=1.5em]
...
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Nguồn gốc Dữ liệu:} Dữ liệu bệnh nhân lâm sàng thu thập từ Bệnh viện Frankfurt, Cộng hòa Liên bang Đức (\href{https://www.kaggle.com/datasets/johndasilva/diabetes}{\texttt{Kaggle -- Diabetes Dataset Hospital Frankfurt}}).
...
\subsection{Minh họa Thực nghiệm trên Giao diện Ứng dụng Web \& Mobile App Lâm sàng}

Nhằm kiểm chứng khả năng tích hợp thực tế, hệ thống đã được triển khai hoàn chỉnh thành ứng dụng đa nền tảng (Web Desktop \& Mobile App Simulator) tương tác thời gian thực (\texttt{app.py}). Các hình ảnh dưới đây minh họa trực quan quy trình chẩn đoán khép kín đối với một ca bệnh nhân nguy cơ cao ($50$ tuổi, $\text{Glucose} = 180\,\text{mg/dL}$, $\text{BMI} = 38.0\,\text{kg/m}^2$, $\text{Insulin} = 220\,\mu\text{U/mL}$):

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{images/web_ui_diagnosis.png}
\caption{Giao diện Web Desktop: Nhập liệu Hồ sơ Y sinh và Kết quả Chẩn đoán Cá thể từ Mô hình AI XGBoost}
...
\end{subfigure}
\caption{Hệ sinh thái Can thiệp Y tế và Mạng lưới Đồ thị Tri thức FPT Long Châu trên Web Desktop}
...
\begin{subfigure}[b]{0.31\textwidth}
    \includegraphics[width=\textwidth]{images/mobile_ui_diabetes_input.png}
    \caption{Giao diện Nhập liệu Y sinh}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.31\textwidth}
    \includegraphics[width=\textwidth]{images/mobile_ui_diabetes_result.png}
    \caption{Kết quả Sàng lọc \& Lời khuyên}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.31\textwidth}
    \includegraphics[width=\textwidth]{images/mobile_ui_diabetes_products.png}
    \caption{Hệ sinh thái Dược phẩm Long Châu}
\end{subfigure}
\caption{Trực quan hóa Giao diện Ứng dụng Di động (Mobile App View) Hệ thống Chẩn đoán Tiểu đường}
\label{fig:mobile_ui_diabetes}
\end{figure}

\textbf{Quy trình vận hành khép kín trên giao diện thực tế (Hình \ref{fig:web_ui_diagnosis}, \ref{fig:web_ui_care_and_kg} \& \ref{fig:mobile_ui_diabetes}):}
\begin{enumerate}[leftmargin=1.5em]
    \item \textbf{Tiếp nhận Hồ sơ Sinh học Cá thể (Hình \ref{fig:web_ui_diagnosis} \& \ref{fig:mobile_ui_diabetes}a):} Người dùng chọn mẫu hồ sơ hoặc nhập 8 chỉ số y sinh vào form. Trên giao diện Mobile App, các trường dữ liệu được thiết kế dạng thẻ tương tác gắn biểu tượng trực quan, hỗ trợ tự động tính toán chỉ số $\texttt{Glucose\_BMI\_Risk} = 180 \times 38.0 = 6,840$.
    \item \textbf{Suy luận Chẩn đoán Cá thể (\texttt{XGBoost (Tuned)}):} Mô hình tính toán xác suất mắc bệnh đạt $\mathbf{83.9\%}$ (vượt xa ngưỡng sàng lọc tối ưu $\theta \ge 0.25$), phân loại người bệnh vào nhóm \textit{Nguy cơ cao (Diabetic)}, gán mã bệnh học \textbf{ICD-10 E11} và xuất lời khuyên lâm sàng yêu cầu xét nghiệm HbA1c tĩnh mạch ngay (Hình \ref{fig:mobile_ui_diabetes}b).
    \item \textbf{Gói Can thiệp Cá nhân hóa Long Châu (Hình \ref{fig:web_ui_care_and_kg}a \& \ref{fig:mobile_ui_diabetes}c):} Tự động đề xuất danh mục giải pháp y tế: Máy đo đường huyết \textit{Accu-Chek Instant} ($950.000$ đ), Que thử ($360.000$ đ), Sữa y học \textit{Abbott Glucerna 850g} ($865.000$ đ), Viên thảo dược \textit{Diabetna} ($115.000$ đ) và kết nối tức thì tới \textbf{Tổng đài Dược sĩ 1800 6928}.
    \item \textbf{Khám phá Mạng lưới Đồ thị Tri thức Động (Hình \ref{fig:web_ui_care_and_kg}b):} Bảng Canvas 2D trên Web Desktop biểu diễn trực quan toàn bộ liên kết ngữ nghĩa giữa thực thể Bệnh nhân $\longleftrightarrow$ Chỉ số sinh học $\longleftrightarrow$ Kết quả AI ($83.9\%$) $\longleftrightarrow$ Mã bệnh ICD-10 E11 $\longleftrightarrow$ Sản phẩm Long Châu $\longleftrightarrow$ Điểm nhà thuốc gần nhất.
\end{enumerate}
...
\begin{itemize}[leftmargin=1.5em, itemsep=2pt]
    \item \textbf{Mã nguồn Notebook Huấn luyện \& Thực nghiệm (Week 2):} \href{https://github.com/phamtu2x5/INTELLIGENT-SYSTEM-DEVELOPMENT/blob/main/src/Week%202/house_price_prediction/notebooks/01_house_price_analysis.ipynb}{\texttt{01\_house\_price\_analysis.ipynb}}
    \item \textbf{Nguồn gốc Dữ liệu trên Kaggle:} \href{https://www.kaggle.com/datasets/huutri148/vietnam-housing-dataset}{\texttt{Kaggle -- Vietnam Housing Dataset}}
...
\textbf{1. Câu hỏi Thực nghiệm (Research Question):}
\textit{"Làm thế nào để tinh chỉnh không gian siêu tham số của XGBoost Regressor nhằm kiểm soát phương sai khi làm việc với không gian đặc trưng thưa $540$ chiều (bao gồm $9$ biến số liên tục kết hợp $531$ biến giả nhị phân sinh ra từ kỹ thuật One-Hot Encoding cho các thuộc tính Tỉnh/Thành phố, Quận/Huyện, Phường/Xã và pháp lý có tần suất $\ge 10$), đồng thời tối ưu hóa thời gian huấn luyện trên tập dữ liệu thị trường lớn ($N=28,045$)?"}
...
\begin{tabular}{lccccc}
\toprule
\textbf{Cấu hình XGBoost} & \textbf{Train $R^2$ $\uparrow$} & \textbf{Test $R^2$ $\uparrow$} & \textbf{MAE (tỷ) $\downarrow$} & \textbf{RMSE (tỷ) $\downarrow$} & \textbf{MAPE (\%) $\downarrow$} \\
\midrule
...
\toprule
\textbf{Mô hình Hồi quy} & \textbf{Thô ($X_{\text{raw}}$)} & \textbf{Kỹ nghệ ($X_{\text{engineered}}$)} & \textbf{Cải thiện MAE $\downarrow$} & \textbf{Cải thiện $R^2$ $\uparrow$} \\
\midrule
\textbf{Linear Regression} & MAE: $1.0950$ ($R^2=0.5982$) & \textbf{MAE: 1.0077 ($R^2=0.6315$)} & \textbf{-0.0873 tỷ} & \textbf{+0.0333} \\
\textbf{Decision Tree} & MAE: $1.2210$ ($R^2=0.4810$) & \textbf{MAE: 1.1548 ($R^2=0.5157$)} & \textbf{-0.0662 tỷ} & \textbf{+0.0347} \\
\textbf{Random Forest} & MAE: $1.1340$ ($R^2=0.5520$) & \textbf{MAE: 1.0786 ($R^2=0.5865$)} & \textbf{-0.0554 tỷ} & \textbf{+0.0345} \\
\textbf{XGBoost (Tuned)} & MAE: $1.0820$ ($R^2=0.6015$) & \textbf{MAE: 1.0206 ($R^2=0.6329$)} & \textbf{-0.0614 tỷ} & \textbf{+0.0314} \\
\bottomrule
\end{tabularx}
...
\textbf{4. Phân tích Trọng số Đóng góp Đặc trưng (Grouped Feature Importance):}
...
\subsection{Minh họa Thực nghiệm trên Giao diện Ứng dụng Web \& Mobile App Định giá Bất động sản}

Nhằm kiểm chứng khả năng tích hợp thực tế, hệ thống đã được triển khai hoàn chỉnh thành ứng dụng đa nền tảng (Web Desktop \& Mobile App Simulator) tương tác thời gian thực (\texttt{app.py}). Các hình ảnh dưới đây minh họa trực quan quy trình định giá và phân tích đồ thị tri thức đối với một căn nhà phố tại đường Phan Văn Trị, Quận Gò Vấp, TP. Hồ Chí Minh ($85\,\text{m}^2$, $4$ tầng, $4$ phòng ngủ, $4$ phòng tắm, mặt tiền $5\,\text{m}$, đường vào $12\,\text{m}$, sổ đỏ đầy đủ):

\begin{figure}[H]
\centering
\includegraphics[width=0.72\textwidth]{images/web_ui_house_price.png}
\caption{Giao diện Web Desktop: Nhập liệu Thuộc tính Bất động sản và Kết quả Định giá AI XGBoost ($8.13$ tỷ VNĐ)}
...
\end{subfigure}
\caption{Hệ sinh thái Tiện ích Đô thị, Gói Vay Ngân hàng và Mạng lưới Đồ thị Tri thức trên Web Desktop}
...
\centering
\begin{subfigure}[b]{0.31\textwidth}
    \includegraphics[width=\textwidth]{images/mobile_app_house_step1_input.png}
    \caption{Giao diện Nhập liệu Không gian}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.31\textwidth}
    \includegraphics[width=\textwidth]{images/mobile_app_house_step2_price.png}
    \caption{Kết quả Định giá AI Cá thể}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.31\textwidth}
    \includegraphics[width=\textwidth]{images/mobile_app_house_step3_amenities.png}
    \caption{Tiện ích Đô thị \& Gói Vay Bank}
\end{subfigure}
\caption{Trực quan hóa Giao diện Ứng dụng Di động (Mobile App View) Hệ thống Định giá Nhà đất}
\label{fig:mobile_ui_house}
\end{figure}

\textbf{Quy trình vận hành khép kín trên giao diện thực tế (Hình \ref{fig:web_ui_re_valuation}, \ref{fig:web_ui_re_amenities_kg} \& \ref{fig:mobile_ui_house}):}
\begin{enumerate}[leftmargin=1.5em]
    \item \textbf{Tiếp nhận Thuộc tính Bất động sản (Hình \ref{fig:web_ui_re_valuation} \& \ref{fig:mobile_ui_house}a):} Người dùng chọn mẫu căn nhà (Nhà phố Gò Vấp, Liền kề Cầu Giấy, Biệt thự Ecopark, v.v.) hoặc nhập trực tiếp các thông số không gian (vị trí địa lý, diện tích $85\,\text{m}^2$, số tầng, mặt tiền $5\,\text{m}$, độ rộng đường vào $12\,\text{m}$, pháp lý, nội thất) qua hệ thống trường thông tin tối ưu hóa trải nghiệm chạm trên di động.
    \item \textbf{Suy luận Định giá Cá thể (\texttt{XGBoost (Tuned)}):} Mô hình AI suy luận giá trị ước tính đạt $\mathbf{8.13\text{ tỷ VNĐ}}$ (đơn giá $\mathbf{95.6\text{ triệu/m}^2}$), hiển thị nổi bật với các thẻ chỉ số tóm lược về diện tích sàn, quy mô phòng và phân khúc bất động sản (Hình \ref{fig:mobile_ui_house}b).
    \item \textbf{Khai phá Tiện ích Đô thị \& Gói Vay Ngân hàng (Hình \ref{fig:web_ui_re_amenities_kg}a \& \ref{fig:mobile_ui_house}c):} Tự động truy vấn mạng lưới tiện ích trong bán kính $2\,\text{km}$ (Vincom Phan Văn Trị $1.2\,\text{km}$, Công viên Gia Định $2.0\,\text{km}$, BV Quân Y 175 $2.5\,\text{km}$, Tuyến Metro số 2 $1.8\,\text{km}$) và tính toán 3 gói vay mua nhà đối tác: \textbf{Techcombank} (Hạn mức $6.10$ tỷ - $75\%$, trả góp $49.1$ tr/tháng), \textbf{Vietcombank} (Hạn mức $5.69$ tỷ - $70\%$, lãi suất $6.5\%$/năm), \textbf{VPBank} (Hạn mức $6.50$ tỷ - $80\%$).
    \item \textbf{Khám phá Mạng lưới Đồ thị Tri thức Đa tầng (Hình \ref{fig:web_ui_re_amenities_kg}b):} Bảng Canvas 2D trên Web Desktop biểu diễn trực quan toàn bộ liên kết đồ thị tri thức giữa Căn nhà $\longleftrightarrow$ Vị trí Quận/Huyện $\longleftrightarrow$ Cụm tiện ích đô thị xung quanh $\longleftrightarrow$ Định giá AI ($8.13$ tỷ) $\longleftrightarrow$ Bảo lãnh tín dụng Ngân hàng ($6.10$ tỷ) $\longleftrightarrow$ Nền tảng niêm yết OneHousing/Batdongsan.
\end{enumerate}
